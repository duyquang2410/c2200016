...
<section>
    <div class="container flex">
        <div class="left">
            ...
            <!-- Hiển thị các ảnh chi tiết từ bảng thư viện ảnh --> 
        </div>
        ...
        <div class="right">
            ...
            <h5>Chất Liệu: <span p class="chatlieu"><?php echo $row['Loai_CL'];?></span></h5>
            <h5>Danh Mục: <span p class="danhmuc"> <?php echo $row['Ten_DM']; ?></span> </h5> <!-- Hiển thị tên danh mục sản phẩm -->
            <h5>Giá: <span p class="gia"><?php echo number_format($row['gia']); ?> VND </span></h4>
    ...
    <span id="plus">+</span>
            ...
            <a id="addToCartBtn" href="dathang.php?id=<?php echo $sp_id; ?>&quantity=" class="btn btn-primary">Mua Ngay</a>
            </div>

            
            <?php
                // Thực hiện truy vấn SQL để lấy các ảnh từ bảng thư viện ảnh có id sản phẩm tương ứng
                $sql_images = "SELECT tenhinh FROM thuvien_anh WHERE Ma_SP = $sp_id";
                $result_images = mysqli_query($conn, $sql_images);

                // Hiển thị các ảnh từ bảng thư viện ảnh
                if (mysqli_num_rows($result_images) > 0) {
                    echo '<div class="thumbnail_images">';
                    while ($image = mysqli_fetch_assoc($result_images)) {
                        echo '<img src="' . $image['tenhinh'] . '" class="thumbnail" width="50%">';
                    }
                    echo '</div>';
                } else {
                    echo "Không có ảnh chi tiết cho sản phẩm này.";
                }
            ?>
        </div>
    </div>
